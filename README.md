...
### User accounts
- `POST /api/users` register a new account (returns an `id` for seating human players)
- `POST /api/users/login` login and receive a short-lived token (in-memory) along with the user `id`
